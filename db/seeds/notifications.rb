...
Ticket.find_each do |ticket|
  ...
  creator, notifiable, description, = case ticket.status
                                      when "approved"
                                      ...
                                      when "manager_feedback"
                                        ...
                                        unless %w[new approved manager_feedback].include?(ticket.status)
                                          formatted_process_by = ticket.process_by.strftime("%Y-%m-%d")
...
OrderItem.find_each do |order_item|
  date = random_date_within_last_8_months
  notification_status = determine_status_based_on_age(date)
  user, notifiable, description, = case order_item.status
                                   when "delayed"
...
Order.find_each do |order|
  date = random_date_within_last_8_months
  notification_status = determine_status_based_on_age(date)
  user, notifiable, description, = case order.status
                                   when "pending"
